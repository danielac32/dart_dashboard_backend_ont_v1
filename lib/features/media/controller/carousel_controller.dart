...
class CarouselController{
  ...
  Future<Map<String, dynamic>> remove(HttpRequest request, HttpResponse response) async {
    ...
    try {
      final imageFile = File('assets/carrusel/$name');
  ...
  Future<Map<String, dynamic>> list(HttpRequest request, HttpResponse response) async {
    try {
      final directory = Directory('assets/carrusel');

      // Verificar si el directorio existe
      if (!await directory.exists()) {
        throw AlfredException(404, 'Carrusel directory not found');
      }
...
// Helper para verificar extensiones de imagen
  ...
  // Helper para validar extensiones (opcional)
  static bool _isValidImageExtension(String filename) {
